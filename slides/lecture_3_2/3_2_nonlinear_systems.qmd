...
$$
\begin{align}
f_1(p_1, p_2) & = c_1 p_1^{\sigma_1} - y \, p_1^{\epsilon_{11}} p_2^{\epsilon_{12}} = 0 \\
f_2(p_1, p_2) & = c_2 p_2^{\sigma_2} - y \, p_1^{\epsilon_{21}} p_2^{\epsilon_{22}} = 0
\end{align}
$$
...
$$
\begin{align}
g_1(z_1, z_2) & = c_1 e^{\sigma_1 z_1} - y \, e^{\epsilon_{11} z_1 + \epsilon_{12} z_2} = 0 \\
g_2(z_1, z_2) & = c_2 e^{\sigma_2 z_2} - y \, e^{\epsilon_{21} z_1 + \epsilon_{22} z_2} = 0
\end{align}
$$
...
[**Better initial guess**]{.blue}: simple, requires no code changes to $f$, but needs problem-specific insight

[**Different method**]{.blue}: Newton with autodiff is more robust than Broyden with finite differences, at the cost of more computation per step

[**Log transformation**]{.blue}: most robust, works from distant guesses, but adds a layer of indirection

. . .

**In practice**, combine both: solve a simplified problem, then use its log-transformed solution as initial guess for the full system

. . .

*A related transformation for variables in $(0,1)$ (e.g., probabilities or shares):*
