---
title: "ACE 592 - Lecture 5.1"
subtitle: "Function approximation: Interpolation"
...
format:
  revealjs: 
    output-file: ACE592_5_1_slides.html
    ...
    chalkboard:
      ...
      chalk-effect: 0.1
  html:
    output-file: ACE592_5_1_notes.html
    toc: false
...
- Today, we will cover function approximation techniques, which are used to estimate unknown functions from data
- These are useful for when we need to evaluate functions at points where we don't have data
...
What's an example of problems with $n$ nodes and $k < n$ basis functions?
...
**1 - Spectral methods**: These methods use basis functions that are nonzero over the entire approximation domain (except for a limited number of points)
...
**2 - Finite element methods**: Basis functions are nonzero only over subsets if the approximation domain
...
gr();
f(x) = sin(x);
plot(f, 0, 2pi, line = 4, grid = false, legend = false)
```
...
println("Coefficients for n = 4: ", f_hat_4.coeffs)
println("Coefficients for n = 5: ", f_hat_5.coeffs)
println("Coefficients for n = 10: ", f_hat_10.coeffs)
```
...
```{julia}
#| code-fold: true
plot(f, 0, 2pi, line = 4, grid = false, label = "sin(x)",
...
Let's try approximating **Runge's function**: `f(x) = 1/(1 + 25x^2)`
...
```{julia}
#| code-fold: true
plot(runge, -1, 1, line = 4, grid = false,
           label = "1/(1 + 25x^2)",
           legendfont = font(7), legend = :topright, ylims = (-.5, 1),
           xlabel = "x", ylabel = "f(x) = 1/(1 + 25x^2)")
```
...
#| code-fold: true
plot(runge, -1, 1, line = 4, grid = false,
           label = "1/(1 + 25x^2)", legend = :top, ylims = (-2, 10),
           ylabel = "f(x) = 1/(1 + 25x^2)", xlabel = "x")
```
...
and are defined on the domain $[-1,1]$. In practice this is easy to expand to any interval $[a,b]$
...
They also have easy derivatives and integrals (see Miranda and Fackler Ch. 6.2 for the expression)
...
## [Chebyshev interpolation in Julia]{.blue}
...
```{julia}
#| code-fold: true
Plots.plot(f, 0, 8, line = 4, grid = false, label = "f(x)",
           xlabel = "x", ylabel = "f(x)", legend=:bottomright)
```
...
So the error in the approximation is bounded, too!
...
John P. Boyd^[Professor at U of Michigan and author of the book "Chebyshev and Fourier Spectral Methods."] summarizes decades of experience with function approximation with his moral principle:
...
If we recover a set of values at our interpolation nodes, we can then simply invert $\Phi$ and right multiply it by $y$ to recover our coefficients ( $c = \Phi^{-1}y$ ) 
...
These are simply the roots of the Chebyshev polynomials above on the domain $[-1,1]$. They are given by
...
plot(cheb_nodes(3), zeros(3, 1), label = "3 Nodes", ylims = (-.01, .01),
    seriestype = :scatter, xlabel = "Chebyshev Nodes", ylabel = "", grid = false, markersize = 5)
```
...
- If we moved to an area closer to the edge of the domain, there may be only one node to the left or right of it providing information on what the value of our approximant should be
- Therefore, it's best to put more nodes in these areas to compensate for this informational deficit and get good approximation quality near the edges of our domain
...
We approximate functions of some arbitrary dimension $N$ by taking the tensor product of vectors of the one-dimensional Chebyshev polynomials
...
- There are a methods to mitigate this problem using, for example, sparse and adaptative grids (e.g., [Smolyak grids](https://robertdkirkby.github.io/IntroToChebyshevSmolyak.html))
...
- In numerical models we typically don't use linear splines because we often care about the quality of approximation of higher order derivatives $\rightarrow$ cubic splines are more common
...
Conditions 1, 2, and  3 combined give us a total of $4N-2$ equations for $4N$ coefficients. We need two more conditions to solve the problem
...
There are a few good packages for splines in Julia: `Interpolations`, `GridInterpolations`, `Dierckx`. We'll see a simple example with `Interpolations` to interpolate `sin(x)` again
...
```{julia}
#| code-fold: true
plot(f, 0, 2pi, line = 4, grid = false, alpha = 0.7, label="Sin(x)")
```
...
```{julia}
#| code-fold: true
plot!(xs, f_linear_spline, color = :green, linewidth = 4.0, linestyle = :dashdot, label="Linear spline")
```
...
```{julia}
#| code-fold: true
plot!(f_cubic_spline, 0, 2pi, color = :red, linewidth = 4.0, linestyle = :dash, label="Cubic spline")
```
